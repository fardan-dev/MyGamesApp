...
//  MyGames
//
//  Created by Muhamad Fardan Wardhana on 08/07/23.
//
...
enum Endpoints {
  enum Gets: Endpoint {
    case games
    case developers
    case detailGame(id: String)
    
    public var url: String {
      switch self {
      case .games: return "\(APICall.baseURL)games"
      case .developers: return "\(APICall.baseURL)developers"
      case .detailGame(let id): return "\(APICall.baseURL)games/\(id)"
      }
    }
  }
}
